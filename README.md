...
### ce.ipynb  
Cross‑entropy generalizes BCE to multi‑class classification. It measures the divergence between a predicted probability distribution 𝑝 and a true one‑hot label 𝑡, penalizing confident but incorrect predictions. The notebook derives CE from first principles and implements it with a custom autograd backward function.
